...
public class MainDialog : IDialog<BasicForm>
{
    ...
    private async Task FormComplete(IDialogContext context, IAwaitable<BasicForm> result)
    {
        try
        {
            BasicForm form = await result;
            if (form != null)
            {
                var clientProxyObject = Configure();
                clientProxyObject.SaveAmendmentDetailstoMSL(new MS.IT.Ops.Shared.Common.StandardInput(), new MS.IT.Ops.ContractManagement.Agreements.BusinessObjects.Agreement());
